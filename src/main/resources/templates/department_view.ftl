...
<#--        新的字段应用ID-->
        <div class="layui-form-item" style="padding-right: 50px">
            ...
            <div class="layui-input-block">
                <input id="dept-name" type="text" name="name" required  lay-verify="required" placeholder="如: （蚂蚁007，阿里996）修福报" autocomplete="off" class="layui-input">
            </div>
        </div>
<#--        签名-->
        <div class="layui-form-item" style="padding-right: 50px">
            ...
            <div class="layui-input-block">
                <input id="dept-description" type="text" name="name" required  lay-verify="required" placeholder="请输入签名, 如: Hariji" autocomplete="off" class="layui-input">
            </div>
        </div>
        ...
        <div class="layui-form-item layui-form-text" style="padding-right: 50px">
            ...
            <div class="layui-input-block">
                <textarea id="dept-sign" name="description" placeholder="请输入备注信息（重要保证）..." class="layui-textarea"></textarea>
            </div>
        </div>
...
<script type="text/html" id="toolbar">
    <div class="layui-btn-container">
        <button class="layui-btn layui-btn-sm" lay-event="addDept">添加订单（支付宝）</button>
    </div>
</script>
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,cols: [[
                ...
                {field:'name', width:150, title: '应用ID', edit: true},
            ...
            switch(obj.event){
                case 'addDept':
                    layer.open({
                        ...
                        area: ["500px", "300px"],
                        yes: function () {
                            const name = $("#dept-name").val();
                            const description = $("#dept-description").val();
                            const sign = $("#dept-sign").val();
                            if (name.length <= 0  || sign.length <= 0 || description.length <= 0 ) {
                                if (name.length <= 0) {
                                    layer.tips('应用ID不能为空', '#dept-name',  {
                                        tipsMore: true
                                    });
                                }
                                if (sign.length <= 0 || description.length <= 0) {
                                    if (sign.length <= 0) {
                                        layer.tips('备注信息不能为空', '#dept-sign',  {
                                            tipsMore: true
                                        });
                                    }
                                }
                                if (description.length <= 0) {
                                    layer.tips('签名不能为空', '#dept-description', {
                                        tipsMore: true
                                    });
                                }
                            } else {
            ...
            if(layEvent === 'edit'){ //编辑
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res.code == 200) {
                            layer.msg('更改支付宝订单信息成功', {icon: 1});
                            obj.update({
                                name: data.name,
                                description: data.description
                            });
                        } else {
                            layer.msg('更改支付宝订单信息失败', {icon: 2});
                        }
                    }
                });
