...
<script type="text/html" id="position-insert">
    <form class="layui-form" method="post">
        <div class="layui-form-item" style="padding-right: 50px">
            <label class="layui-form-label">订单类型名称</label>
            <div class="layui-input-block">
                <input id="position-name" type="text" name="name" required  lay-verify="required" placeholder="请输入订单类型, 如: Java开发工程师" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item layui-form-text" style="padding-right: 50px">
            <label class="layui-form-label">订单类型描述</label>
            <div class="layui-input-block">
...
<script type="text/html" id="toolbar">
    <div class="layui-btn-container">
        <button class="layui-btn layui-btn-sm" lay-event="addPosition">添加订单类型</button>
    </div>
</script>
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,cols: [[
                ...
                {field:'name', width:150, title: '订单类型名称', edit: true},
                {field:'description', width:200, title:'订单类型描述', edit: true},
                    ...
                    layer.open({
                        ...
                        yes: function () {
                            ...
                            if (name.length <= 0  || description.length <= 0) {
                                if (name.length <= 0) {
                                    layer.tips('订单类型名称不能为空', '#position-name',  {
                                        tipsMore: true
                                    });
                                }
                                if (description.length <= 0) {
                                    layer.tips('订单类型描述不能为空', '#position-description', {
                                        tipsMore: true
                                    });
                                }
                            } else {
                                ...
                                $.ajax({
                                    ...
                                    success: function (res) {
                                        ...
                                        if (res.code == 200) {
                                            ...
                                            layer.msg('新建订单类型成功', {icon: 1});
                                        } else {
                                            layer.msg('新建订单类型失败', {icon: 2});
                                        }
                                    }
                                });
                            }
